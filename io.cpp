...
bool IO::getUUID(void) {
  if (currentBackend == IO_SERIAL) {
    ...
    // simple trim:
    while (bytesRead > 0 && address[0] == ' ') {
      memmove(address, address + 1, --bytesRead);
      address[bytesRead] = '\0';
    }
    while (bytesRead > 0 && address[bytesRead - 1] == ' ') {
      address[--bytesRead] = '\0';
    }
...
bool IO::sendModel(float *weights, size_t len) {
  ...
  if (currentBLEMode == WS_BLE_CENTRAL) {
    ...
    while (!attemptConnectionToPeripheral(peripheral) &&
           (millis() - start) < 30000) {
      BLE.poll();
      delay(10);
    }
    ...
    Serial.println(
        "Central: Connection established, waiting before sending...");
    delay(3000);
  }
  ...
  return weightShareSend(currentBLEMode, MSG_TYPE_WEIGHTS, (uint8_t *)weights,
                         len);
...
bool IO::sendNBatches(const uint16_t n_a, size_t len) {
  if (currentBackend == IO_SERIAL) {
    ...
    bool res = weightShareSend(currentBLEMode, MSG_TYPE_BATCH_COUNT,
                               (uint8_t *)&n_a, len);
...
bool IO::receiveModel(float *weights, size_t len) {
  if (currentBackend == IO_SERIAL) {
    ...
    // receive
    return weightShareReceive(currentBLEMode, MSG_TYPE_WEIGHTS,
                              (uint8_t *)weights, len);
  }
  // TODO:
  // fix for bluetooth computer connection as well...
...
bool IO::receiveNBatches(uint16_t *n_b, size_t len) {
  if (currentBackend == IO_SERIAL) {
    ...
    return weightShareReceive(currentBLEMode, MSG_TYPE_BATCH_COUNT,
                              (uint8_t *)n_b, len);
  }
  // TODO:
  // fix for bluetooth computer connection as well...
