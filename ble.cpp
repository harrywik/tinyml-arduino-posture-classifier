...
BLECharacteristic sensorCharacteristic(BLE_CHARACTERISTIC_UUID, permissions,
                                       64);
...
void readvertiseBLE(void) { BLE.advertise(); }
...
bool BLEReceive(String &cmd) {
  ...
  if (sensorCharacteristic.written()) {
    cmd = String((char *)sensorCharacteristic.value(),
                 sensorCharacteristic.valueLength());
    return true;
  }
  return false;
}
