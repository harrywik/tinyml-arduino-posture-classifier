...
bool shareW_out(uint16_t *nBatchesOnDevice) {
  ...
  if (Coms.getUUID()) {
    ...
    Serial.print(W_a.weights[0][0]);
    Serial.print(", ");
    Serial.print(W_a.weights[0][1]);
    Serial.print(", ");
    ...
    // then receive
    if (!Coms.receiveModel((float *)W_b.weights,
                           sizeof(float) * W_out_length)) {
    ...
    Serial.print(W_b.weights[0][0]);
    Serial.print(", ");
    Serial.print(W_b.weights[0][1]);
    Serial.print(", ");
  ...
  for (size_t i = 0; i < OUTPUT_SIZE; i++) {
    for (size_t j = 0; j < RESERVOIR_SIZE; j++) {
      ...
      W_a.weights[i][j] =
          (W_a.weights[i][j] * (float)n_a + W_b.weights[i][j] * (float)n_b) /
          (float)n_tot;
