...
bool setKVPersistedWeights(float W_out[OUTPUT_SIZE][RESERVOIR_SIZE]) {
  // kv_set requires the data address to be cast to a uint8_t pointer (raw
  // bytes). The array name 'W_out' decays to the address of the first element.
...
bool rmKVpersistedWeights(void) { return kv_remove(W_OUT_KEY) == KV_R_OK; }
...
bool rmKVpersistedEMA(void) { return kv_remove(EMA_KEY) == KV_R_OK; }

// Fetch current current number of batches processed
bool getNProcessedBatches(uint16_t *nBatches) {
  size_t actual_size;
  size_t ret =
      kv_get(BATCH_KEY, (uint8_t *)nBatches, BATCH_N_SIZE, &actual_size);
...
bool rmNProcessedBatches(void) { return kv_remove(BATCH_KEY) == KV_R_OK; }
